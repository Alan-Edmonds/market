...
filenames = ['OptionTradeScreenerResults_20200608',
...
def all_combiner(input): #this writes the all_combiner123123.csv, which is all of the block trades near ask from all the input files
    data = []
    trade_day = 505
    ...
    with open('all_combiner123.csv', 'w') as f:
        writer = csv.writer(f)
        print("writing data into all_combiner123.csv...")
        for row in tqdm(data):
            writer.writerow(row)
#all_combiner(filenames)

#this reads in all_combiner123.csv and writes the buy trades into buys123.csv. Also calculates and appends a few more variables for each trade:
...
def filter_buys():
    rows = []
    print("Reading all_combiner123.csv. Running filter_buys()...")
    with open('all_combiner123.csv') as f:
        reader = csv.reader(f)
        for row in tqdm(reader):
            ...
            if row[20] != '' and row[22] != '':
                ...
                row.append('')
            rows.append(row)
    with open('buys123.csv', 'w') as f:
        writer = csv.writer(f)
        print("writing data into buys123.csv...")
...
def price_history(): #return value lets us see how the price of a specific option changed
    ...
    print("Reading all_combiner123.csv. Running price_history()...")
    with open('all_combiner123.csv') as f:
        reader = csv.reader(f)
        for row in tqdm(reader):
            if row == []:
...
#this function reads in buys123.csv and for each buy trade, calculates a score for that trade based on how the price of
...
def scorer():
    ...
    with_scores = []
    with open('buys123.csv') as f:
        reader = csv.reader(f)
        print('reading buys123.csv. Running scorer()...')
        count = 0
        for row in tqdm(reader):
            count += 1
    ...
    with open('buys_with_scores123.csv', 'w') as f:
        writer = csv.writer(f)
...
def grouped_attributes(input):
    ...
    if attributes == "Specific attributes: ":
        attributes = "Specific attributes:   None. All trades from buys_with_scores123.csv"
    #print(attributes)
    ...
    with open('buys_with_scores123.csv') as f:
        reader = csv.reader(f)
