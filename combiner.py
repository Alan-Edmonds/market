...
def all_combiner(input): #this writes the combined.csv, which is all of the block trades near ask from all the input files
    ...
    with open('combined.csv', 'w') as f:
        writer = csv.writer(f)
        print("writing data into combined.csv...")
        for row in tqdm(data):
            writer.writerow(row)
#all_combiner(filenames)

#this reads in combined.csv and writes the buy trades into buys.csv. Also calculates and appends a few more variables for each trade:
...
def filter_buys():
    rows = []
    print("Reading combined.csv. Running filter_buys()...")
...
def price_history(): #return value lets us see how the price of a specific option changed
    ...
    print("Reading combined.csv. Running price_history()...")
    with open('combined.csv') as f:
        reader = csv.reader(f)
        for row in tqdm(reader):
            if row == []:
